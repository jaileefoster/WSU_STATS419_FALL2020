...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
Looking at the data visualized in this way, my initial perspective is that the counts of many of the keywords look to be increasing slightly at a relatively constant rate. Another observation that I made is that the keywords with higher counts, such as Microsoft Office, C++, and Data Entry, seem to be more generalized skills, likely to be used in a variety of jobs, whereas the keywords with lower counts, such as Power BI, Stata, and MariaDB, are skills that are more specific to certain niches in the Data Analytics field. "Parallel-ish" lines on this plot signify that the rate at which each job count is changing is approximately proportional to the rates that the job counts for other keywords are changing. From this chart, it does not seem that most of the job counts are changing very drastically, but because the job counts for each keyword do generally seem to be increasing, this may represent a growing job market and increased job demand in the field. These lines are following an increasing, or positive, trend. 

The line representing the data "Data analysis" seems to be following the trend of many of the other lines. Although the job counts are not increasing at a constant rate over the 5 weeks, they are generally increasing at some rate. One thing that I did notice is that between weeks 40 and 41 there seems to be a bit of a slump in the line where the count did not seem to increase as drastically as it did between other weeks. Compared to other Search-Query words, "Data analysis" seems to increase at a faster rate in some weeks, and at a slower week some weeks, but overall it follows the same trend as the other words.
</pre>
...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
It seems highly unlikely (almost impossible) that the count for the "Git" search-query dropped to 0 in week 40, especially considering that the weeks before and after this week have counts of 18,617 and 19,528, respectively. A more probable explanation for the sudden, drastic drop is missing data. There are multiple ways to deal with missing data, but in this case, because the trend line is increasing somewhat constantly, I think it would be appropriate to average the data for the two weeks before and after the missing data point and use that number for the missing data. We could also just fill that specific point with either the count that happened directly before or directly after the missing point. With this data set in particular, I noticed that there are counts for both "Git" and "git", and the counts for a given week are extremely similar. Because of this, it also seems like a valid solution would be to use the "git" count for week 40 to replace the missing data point.  
</pre>
...
## I chose to change this to an average of the counts from weeks 38, 39, 40, and 41, which is 19090
...
summary(jobs.subset[jobs.subset$search.query == 'Microsoft Office', ]$job.count)

summary(jobs.subset[jobs.subset$search.query == 'C++', ]$job.count)
...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
We can conclude that the data are different based on both "mathematics" and the "boxplot". Above, I performed summary statistics on the "job.count" column for both the "Microsoft Office" and "C++" search-queries and the results were consistent with the boxplots. Visually, the boxplots show us that there is not any overlap in the data, and the summary statistics prove this same idea mathematically. The minimum value for the "Microsoft Office" job count, 22,3663, was higher than the maximum value for the "C++" job count, 16,9786, further showing that there is not any overlap between the data of the two search-queries. 

In this case, I do not believe that a formal inferential statistical test would tell us something different than logical inference. Although this is not always the case, the distance between "Microsoft Office" data and "C++" data is significant enough that it seems intuitive to see that there is significant difference between the data. I think that formal inferential statistical tests were derived from a combination of mathematics, boxplots, and other mathematical and statistical models. 
</pre>
...
summary(jobs.subset[jobs.subset$search.query == 'Statistics', ]$job.count)
summary(jobs.subset[jobs.subset$search.query == 'Java', ]$job.count)
...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
The interpretation of the question "Is 'Statistics' bigger than 'Java'?" may vary depending on who is answering the question, but I have chosen to interpret it as such: "Are the job counts for 'Statistics' significantly larger than those of 'Java'?" Based on "mathematics", "boxplot". and "ttest", it is not reasonable to conclude that there is significant difference between the job counts of "Statistics" and "Java", therefore, we cannot say that "Statistics" is bigger than "Java". As stated above, there are points where the summary statistics of "Statistics" exceed those of "Java", but it is also true the other way around. Visually, the boxplot shows much less difference between the two keywords that we saw in the above example. In fact, the data for "Java" seems to lay inside the range of the data for "Statistics". Furthermore, looking at the t-test for the job counts of these two search-queries, we see that the p-value for this specific test is 0.6333. Thus, we fail to reject the null hypothesis (the data between the two keywords is not significantly different), and cannot conclude significant difference between the two data sets. 
</pre>
...
summary(jobs.subset[jobs.subset$search.query == 'Data science', ]$job.count)
summary(jobs.subset[jobs.subset$search.query == 'Big data', ]$job.count)
...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
As in the previous example, it does not seem plausible to draw the conclusion that there is significant difference between the data for these two keywords based on the boxplot and summary statistics of the two. When the t-test is performed with a null hypothesis that the data between the two keywords is not significantly different, it results in a p-value of 0.3096. In this case, we also fail to reject the null hypothesis, so we cannot conclude that the data are statistically different. 

The overlap between the keywords "Data science" and "Big data", and many of the other keywords as well, is definitely something that should be paid attention to when dealing with this data. Many of the words that data is being collected for are very closely related, and may even be used interchangeably in some instances. When dealing with the keywords "Data science" and "Big data" in particular, I would imagine that many of the job postings that contain the keyword "Big data" would also contain "Data science", as the idea of big data is a subset of the data science field. 

The other comparisons that were performed may be "very likely okay" because the keywords that are being compared are less likely to result in overlap. The keywords "Microsoft Office" and "C++", and "Statistics" and "Java" are not as closely related to each other as the keywords "Data science" and "Big data" are.

An improved approach to collecting the data that may allow more accurate comparison of these two keywords would be to also collect the number of job postings that include both keywords. This way, we would be able to see not only how many job postings possess both of the keywords, but also how many include only one or the other of these keywords. 
</pre>
