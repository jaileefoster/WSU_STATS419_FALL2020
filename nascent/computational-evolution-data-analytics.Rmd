...
This algorithm works by picking one number out of the unsorted vector, and placing it into the correct place in the sorted vector. This is much like choosing an item of clothing, folding it, then putting it away where it belongs, hence the name laundry sorting. Given the input, the number of tasks that need to be performed to sort using this approach is the input squared. The stopping rule may be improved by first iterating over the parameter list to see if it is already sorted. If it is, no work needs to be done. 
...
Computing both the minimum and the maximum at the same time decreases the time complexity of this search algorithm.
...
As expected, the swap-meet (bubble) sort is the least efficient, this is because it performs a larger number of comparisons and swaps than the others. The built in default sort is the quickest. This seems to make sense because when using R, we generally want the most efficient algorithms to be the default.
