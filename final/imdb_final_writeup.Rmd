...
keywords: |
    Will Smith; Denzel Washington; Internet Movie Database; IMDb; simulated critical points
abstract: |
...
•	this notebook is found in the nascent folder
•	the concept of where we are with data analytics is a function of our culture and society
•	we need to let go of preconceptions and let the data speak – culture changes over time
•	the evolution of mathematics is the evolution of making computation easier
•	computation has changed significantly, but we cannot forget the past as we go into the future
•	the naïve approach is very efficient compared to the two-step approach
•	the mean is not one of the numbers selected in the data – it is an artificial number that represents the center
•	it is easier to compute the mean than the median – you don’t have to sort
•	sorting is more computationally difficult
•	John Tuckey realized that if we can sort the data, we can look at the data in a different way
•	with sorting, we have to hold everything in memory
•	laundry sorting – choose one number at time and place it where it belongs
•	selection sort – loop through all of the data to find the minimum and the maximum, then move the min and max to the new vector
•	swap-meet sort (bubble sort) – go left to right and compare two numbers at a time and swap them as needed. This will push the biggest number to the end in one iteration. 
•	default sort method in r uses the radix method – best for under 2 billion numbers
•	Median – the middle number of a sorted list of numbers, not the average of the two middle. Pick either the first or the second number based on an algorithm
•	Algorithm 1 in R gives numbers that are actually in the data
•	MAD – median absolute deviation. Shows how much dispersion / spread from the median
•	Find the median, take deviations of the median (distances of each number from the median), then find the median of the deviations
•	If the data is normal…. mean = median = mode
...
measure = utils::read.csv( paste0(path.to.secret, "measure-students.txt"), header=TRUE, 
                           quote="", sep="|");
